...
The mock user's identity provider defaults to `IDIR`. To exercise BCeID's page restrictions locally, set the backend's `AUTH_MOCK_IDP=BCEIDBUSINESS` in `.env`, and use the IDP switcher next to the mock role switcher in the frontend header (stored in `localStorage` under `csp.mockIdp`).
...
> **BCeID login.** The welcome screen (`/`) offers both "Log in with IDIR" and "Log in with BCeID" buttons. IDIR redirects via the configured `idpName` Cognito custom identity provider; BCeID's is instead derived at runtime as `${appEnv}-BCEIDBUSINESS` (e.g. `DEV-BCEIDBUSINESS`) — no separate config value, matching the convention used by the nr-scs/nr-waste-plus sibling apps. FAM must have registered a Cognito custom IdP under that exact name for BCeID (`oidc_clients_csp.tf`). BCeID users authenticate as external clients and are restricted, both in the UI (hidden nav, redirected routes) and at the API (`SecurityConfig`, `hasAuthority("IDP_IDIR")`), to only the Upload Submission and Submission History pages.
