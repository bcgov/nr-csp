...
@RequestMapping("/api")
public interface R10Api {

    @Operation(summary = "Generate R10 report", description = "Generates the R10 invoice report in PDF or CSV format.")
    @ApiResponses({
    ...
    @PostMapping(value = "/R10",
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = {MediaType.APPLICATION_PDF_VALUE, "text/csv", MediaType.ALL_VALUE})
    ResponseEntity<Resource> generateR10Report(@Valid @RequestBody R10ReportRequest request);
}
