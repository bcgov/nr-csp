...
class R11ControllerTest {

    @Mock
    R11Service r11Service;
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders
                .standaloneSetup(new R11Controller(r11Service))
    ...
    @Nested
    @DisplayName("POST /api/R11")
    class PostR11 {

        @Test
        void shouldReturn200_whenValidRequest() throws Exception {
            given(r11Service.generateReport(any()))
                    .willReturn(new ReportResult(new byte[]{1}, "R11_test.pdf"));

            mockMvc.perform(post("/api/R11")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"reportFormat\":\"PDF\",\"dateFrom\":\"20200101\",\"dateTo\":\"20201231\"}"))
                    .andExpect(status().isOk())
                    .andExpect(header().string("Content-Disposition", "attachment; filename=\"R11_test.pdf\""));
        }
        ...
        void shouldReturn400_whenDateFromInvalidFormat() throws Exception {
            mockMvc.perform(post("/api/R11")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"reportFormat\":\"PDF\",\"dateFrom\":\"01/2020\"}"))
        ...
        void shouldReturn400_whenDateToInvalidFormat() throws Exception {
            mockMvc.perform(post("/api/R11")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"reportFormat\":\"PDF\",\"dateTo\":\"2020-12-31\"}"))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
        }

        @Test
        void shouldReturn200_whenNoDatesProvided() throws Exception {
            given(r11Service.generateReport(any()))
                    .willReturn(new ReportResult(new byte[]{1}, "R11_test.pdf"));
