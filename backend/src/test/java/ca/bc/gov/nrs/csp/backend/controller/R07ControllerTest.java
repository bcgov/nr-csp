...
class R07ControllerTest {

    @Mock
    R07Service r07Service;
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders
                .standaloneSetup(new R07Controller(r07Service))
    ...
    @Nested
    @DisplayName("POST /api/R07")
    class PostR07 {

        @Test
        void shouldReturn200_whenValidRequest() throws Exception {
            given(r07Service.generateReport(any()))
                    .willReturn(new ReportResult(new byte[]{1}, "R07_test.pdf"));

            mockMvc.perform(post("/api/R07")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"reportFormat\":\"PDF\",\"year\":2020,\"month\":1}"))
                    .andExpect(status().isOk())
                    .andExpect(header().string("Content-Disposition", "attachment; filename=\"R07_test.pdf\""));
        }
        ...
        void shouldReturn400_whenSubmissionNumberNotNumeric() throws Exception {
            mockMvc.perform(post("/api/R07")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"reportFormat\":\"PDF\",\"submissionNumber\":\"ABC\"}"))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
        }
        ...
        void shouldReturn400_whenSubmissionYearMonthInvalidFormat() throws Exception {
            mockMvc.perform(post("/api/R07")
                            .contentType(MediaType.APPLICATION_JSON)
                            .content("{\"reportFormat\":\"PDF\",\"submissionYearMonth\":\"2020\"}"))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"));
        }
    }
