...
class SchemaValidatorTest {
  ...
  void validate_wraps_pre_parse_sax_failure_as_xml_parse() throws Exception {
    ...
    assertThat(errors.getFirst().message()).isEqualTo("pre-parse failure");
  }
  ...
  void validate_wraps_io_failure_as_xml_read() throws Exception {
    ...
    assertThat(errors.getFirst().message()).isEqualTo("stream broke");
  }
  ...
  void collectingErrorHandler_warning_does_not_collect() throws Exception {
    Class<?> c = Class.forName(
        "ca.bc.gov.nrs.csp.backend.submission.structural.schema.SchemaValidator$CollectingErrorHandler");
  ...
  private static Object newResolver(String schemaRoot) throws Exception {
    Class<?> c = Class.forName(
        "ca.bc.gov.nrs.csp.backend.submission.structural.schema.SchemaValidator$ClasspathLsResolver");
