...
import { getClientsByNumber, type ClientLocationResponse } from '@/services/search.service';
...
const itemToString = (item: ClientLocationResponse | null | undefined): string => item?.clientNumber ?? '';

const renderItem = (item: ClientLocationResponse): string => `${item.clientNumber} - ${item.clientName}`;

const ClientNumberAutocomplete = ({
  id,
  titleText,
  disabled,
  size = 'lg',
  ...
  return (
    <AutoCompleteInput<ClientLocationResponse>
      ...
      hideLabel={hideLabel}
      placeholder=""
      onAutoCompleteChange={getClientsByNumber}
...
export default ClientNumberAutocomplete;
